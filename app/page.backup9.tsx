...
const getStableHue = (text?: string) => {
  const first = getFirstGrapheme(text);
  const cp = first.codePointAt(0) ?? 65;
  return (cp * 17) % 360;
};
  // ── 渲染 ──
  return (
    <div style={{ display: "flex", height: "100vh", width: "100vw", background: "#F0F2F5", fontFamily: "'PingFang SC','Microsoft YaHei',sans-serif", overflow: "hidden", position: "relative", color: "#333" }}>
        ...
        <div style={{ position: "fixed", top: 0, left: 56, right: 0, zIndex: 5000, background: "linear-gradient(90deg,#FF4D4F,#FF7875)", color: "#fff", padding: "10px 20px", display: "flex", alignItems: "center", gap: 12, boxShadow: "0 4px 20px rgba(255,77,79,0.5)" }}>
          ...
          <button onClick={() => { setAiPanelVisible(false); setRightTab("priority"); }} style={{ padding: "5px 12px", borderRadius: 8, background: "rgba(255,255,255,0.2)", border: "1.5px solid rgba(255,255,255,0.6)", color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>去优先级确认 →</button>
            ...
            return (
              <div key={msg.id}>
                ...
                <div style={{ display: "flex", flexDirection: msg.self ? "row-reverse" : "row", alignItems: "flex-start", gap: 8, marginBottom: 10 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 6, flexShrink: 0, background: msg.isAI
  ? "linear-gradient(135deg,#4A90D9,#9B59B6)"
  : msg.self
    ? "linear-gradient(135deg,#4A90D9,#7B68EE)"
    : `hsl(${getStableHue(msg.sender)},60%,60%)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff" }}>
                    {msg.isAI ? "Q" : msg.self ? "我" : getFirstGrapheme(msg.sender)}
                  </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ flex: 1, padding: "0 12px 12px", display: "flex", flexDirection: "column", gap: 8 }}>
              {capsules.map(cap => {
                ...
                {/* ─ 优先级面板 ─ */}
                {!aiPanelVisible && rightTab === "priority" && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ padding: "12px 14px 8px", background: "linear-gradient(135deg,#FFF7E6,#FFF0F5)", borderBottom: "1px solid #F0F0F0", flexShrink: 0 }}>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "8px 12px", display: "flex", flexDirection: "column", gap: 6 }}>
              {(()=>{
                ...
                schedule.filter(ev => !skipTitles.some(s => ev.title.includes(s))).forEach(ev => {
                  ...
                  if (ev.date <= t3) items.push({ id:key, emoji:"🔴", level:"紧急",   color:"#FF4D4F", title:ev.title, desc });
                  else if (ev.date <= t7) items.push({ id:key, emoji:"🟠", level:"重要",   color:"#FA8C16", title:ev.title, desc });
                  else if (isLow) items.push({ id:key, emoji:"🟡", level:"一般",   color:"#FAAD14", title:ev.title, desc });
                  else items.push({ id:key, emoji:"🟢", level:"已确认", color:"#52C41A", title:ev.title, desc });
                });
                const ord: Record<string,number> = { 紧急:0, 敏感:1, 重要:2, 一般:3, 方案参考:4, 已确认:5 };
                const seen = new Set<string>();
                return items
                  .sort((a,b) => (ord[a.level]??9) - (ord[b.level]??9))
      ...
      <div style={{ position: "fixed", right: 16, bottom: 90, zIndex: 1002, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
        {reminderConfig && (
          <div style={{ background: "#52C41A", color: "#fff", fontSize: 9.5, fontWeight: 700, padding: "2px 7px", borderRadius: 8, whiteSpace: "nowrap", boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }}>
            提醒已设定 {reminderConfig.sendTime}
          </div>
        )}
        <button onClick={() => {
          if (timeJumped) return;
          const rc = reminderConfig || { styleIdx: 0, sendTime: "21:00", evId: -1, evTitle: "📎 计网作业截止" };
          ...
          const styleText = ev
            ? (getReminderStyles(ev)[rc.styleIdx]?.text || `提醒：「${rc.evTitle}」今日截止！`)
            : `亲爱的，「${rc.evTitle}」截止今日 23:59，记得提前提交，加油你可以的！🌸`;
          setTimeJumped(true);
          setActiveChat(9);
          ...
          setTimeout(() => {
            const nm: Message = {
              id: Date.now(), sender: "Q仔", isAI: true,
      ...
      {reminderEvent && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2000 }} onClick={() => { setReminderEvent(null); setSentReminder(false); }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "26px", width: 420, boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ background: "#F8F9FB", border: "1px solid #E5E8EE", borderRadius: 10, padding: "12px 14px", fontSize: 13, color: "#333", lineHeight: 1.6, marginBottom: 12, minHeight: 64 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 6 }}>
                {(["1h","3h","custom"] as const).map(opt => {
                  ...
                  return (
                    <button key={opt} onClick={() => setReminderTimeOpt(opt)}
                      style={{ padding: "4px 10px", borderRadius: 8, border: `1.5px solid ${reminderTimeOpt === opt ? "#4A90D9" : "#E5E8EE"}`, background: reminderTimeOpt === opt ? "#E8F0FE" : "#fff", color: reminderTimeOpt === opt ? "#4A90D9" : "#666", fontSize: 11, cursor: "pointer", fontWeight: reminderTimeOpt === opt ? 700 : 400 }}>
                      {lbs[opt]}
                    </button>
                  );
                })}
              ...
              {reminderTimeOpt === "custom" && (
                <input type="time" value={reminderCustomT} onChange={e => setReminderCustomT(e.target.value)}
                  style={{ height: 32, padding: "0 8px", border: "1px solid #E5E8EE", borderRadius: 6, fontSize: 13, outline: "none", color: "#333", marginBottom: 5 }} />
              )}
              <div style={{ fontSize: 11, color: "#999", marginTop: 2 }}>
                将于 <strong style={{ color: "#4A90D9" }}>{computeSendTime(reminderTimeOpt, reminderCustomT, reminderEvent)}</strong> 自动发送提醒
              </div>
            </div>
            {!sentReminder ? (
              <button onClick={() => {
                const st = computeSendTime(reminderTimeOpt, reminderCustomT, reminderEvent!);
                setReminderConfig({ styleIdx: selectedStyle, sendTime: st, evId: reminderEvent!.id, evTitle: reminderEvent!.title });
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: "26px", width: 380 }} onClick={e => e.stopPropagation()}>
            ...
            {[
              ...
              <div key={i} style={{ background: slot.hot ? "#F5F0FF" : "#FAFBFD", border: `1.5px solid ${slot.hot ? "#7B68EE" : "#E5E8EE"}`, borderRadius: 10, padding: "11px 14px", marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <button onClick={()=>{
                  const dates=["2026-05-02","2026-05-03","2026-05-09"];
                  const sTs=["14:00","10:00","15:00"];
                  const eTs=["16:00","12:00","17:00"];
                  const nev:ScheduleEvent={id:Date.now(),date:dates[i],startTime:sTs[i],endTime:eTs[i],title:"🌸 中山陵出游",type:"event",color:"#FCE4EC",fromCapsule:true,groupName:"小美 💕"};
                  setSchedule(prev=>[...prev,nev]);
                  setScheduleModal(false);
                  showToast("✅ 已写入日程！","#7B68EE");
                  const notice:Message={id:Date.now()+1,sender:"Q仔",isAI:true,content:`📅【出游排期已确认通知】\n\n三人共同出游时间正式确定！\n\n🕐 时间：${slot.time}\n📍 地点：中山陵（地铁2号线中山陵站，步行15分钟）\n👥 参与人：你、小美、小红（3人均已确认）\n\n提醒：记得提前在「钟山风景名胜区」公众号实名预约门票，穿平底鞋，带充电宝！🌸`,time:"12:01",self:false};
                  setMessages(prev=>({...prev,13:[...(prev[13]||[]),notice]}));
